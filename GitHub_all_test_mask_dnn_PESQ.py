...
# Test stage for PMESQ-loss mask-based DNN, generates the magnitude of FFT coeff. for
...
#
#   References:
#    [1] Z. Zhao, S. Elshamy, and T. Fingscheidt, "A Perceptual Weighting Filter Loss for DNN Training in Speech
#    Enhancement", arXiv preprint arXiv:1905.09754.
#    [2] J.M.Martin, A.M.Gomez, J.A.Gonzalez, A.M.Peinado 'A Deep Learning Loss Function based on the Perceptual
...
for k_snr in range(0, len(SNR_situ_array)):
    ...
    recon_file = "./test results/mask_dnn_PESQ_s_hat_snr_" + SNR_situ + "_model_" + noi_situ_model_str + "_test_data.mat"
    ...
    recon_file = "./test results/mask_dnn_PESQ_s_tilt_snr_" + SNR_situ + "_model_" + noi_situ_model_str + "_test_data.mat"
    recon_file = os.path.normcase(recon_file)
    ...
    recon_file = "./test results/mask_dnn_PESQ_n_tilt_snr_" + SNR_situ + "_model_" + noi_situ_model_str + "_test_data.mat"
    recon_file = os.path.normcase(recon_file)
